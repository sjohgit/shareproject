...
public class HueboardExcelbean extends AbstractXlsxView{
	...
	protected void buildExcelDocument(Map modelMap, Workbook workbook, HttpServletRequest request,
		...
		CellStyle style = workbook.createCellStyle(); // 셀 스타일을 위한 변수 style.setAlignment(CellStyle.ALIGN_CENTER); // 글 위치를 중앙으로 설정 List listExcel = (List) modelMap.get("list"); // 새로운 sheet를 생성한다. worksheet = workbook.createSheet("엑셀 목록"); // 가장 첫번째 줄에 제목을 만든다. row = worksheet.createRow(0); // 칼럼 길이 설정 int columnIndex = 0; while (columnIndex < 7) {
		...
		for(BoardDTO dto ; listExcel) { 
			row = worksheet.createRow(rowIndex); 
			row.createCell(0).setCellValue(dto.getNum()); 
			row.createCell(1).setCellValue(dto.getContent()); 
			row.createCell(2).setCellValue(dto.getWriter()); 
			row.createCell(3).setCellValue(dto.getReg_date().toLocaleString()); 
			row.createCell(4).setCellValue(dto.getReadcount()); 
		...
		// 지정한 스타일을 입혀준다. 
		try { response.setHeader("Content-Disposition", "attachement; filename=\"" 
