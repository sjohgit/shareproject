...
<table width="700" cellpadding="0" cellspacing="0" font-size= "12pt"
		align="left">
	<tr>
		<td align="left">
		...
		<c:if test="${count > 0}">
			<tr>
				<td><c:if test="${startPage > 10}">
						<a
							href="/huation/board/List.do?pageNum=${startPage-10}">[이전]</a>
					</c:if> <c:forEach var="b" begin="${startPage}" end="${endPage}" step="1">
						<a href="/huation/board/List.do?pageNum=${b}">[${b}]</a>
					</c:forEach> <c:if test="${endPage < pageCount}">
						<a
							href="/huation/board/List.do?pageNum=${startPage+10}">[다음]</a>
					</c:if></td>
			</tr>
		</c:if>
	</table>
